<aside class="w-50 bg-white text-gray-800 dark:bg-gray-800 dark:text-white h-screen shadow-lg" x-data="{ open: {{ in_array(Route::currentRouteName(), ['menu.submenu1', 'menu.submenu2', 'menu.submenu3']) ? '1' : 'null' }} }">
    <!-- Sidebar Header -->
    ...
    <nav class="mt-3">
        ...
        @cannot('isClient', Auth::user())
            <a href="#"
                class="block px-4 py-3 mb-2 rounded-md transition duration-200 hover:bg-blue-600 hover:text-white cursor-not-allowed opacity-50"> 
                Indicadores
            </a>
        @endcannot
        @cannot('isClient', Auth::user())
