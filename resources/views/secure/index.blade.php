                ...
                <div class="p-4 flex items-center justify-between">
                    ...
                    <div class="flex justify-end space-x-1">
                        ...
                        <a class="block px-2 py-1 text-white transition-all duration-500 ease-in-out transform hover:scale-105"
                            href="{{ route('admin-secure.index') }}"><svg xmlns="http://www.w3.org/2000/svg" fill="none"
                            ...
                            @foreach ($users as $user)
                                <tr id="{{ $user->id }}">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                            class="text-sm font-medium text-gray-900 dark:text-white">{{ $user->id }}</span>
                                    </td>
                                    <td x-text={{ $user->name }} class="px-6 py-4 whitespace-nowrap">
                                        <span
                                            class="text-sm font-medium text-gray-900 dark:text-white">{{ $user->name . ' ' . $user->last_name }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                            class="text-sm font-medium text-gray-900 dark:text-white">{{ $user->email }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                            class="inline-block   mx-auto w-3 h-3 {{ $user->is_connected ? 'bg-green-500' : 'bg-red-500' }} rounded-full"></span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap overflow-hidden text-ellipsis">
                                        <span
                                            class="inline-block w-3 h-3  text-gray-900 dark:text-white rounded-full">{{ $user->GeneralManagement->general_management }}
                                        </span>

                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                            class="text-sm font-medium text-gray-900 dark:text-white">{{ $user->jobTitle->title }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
